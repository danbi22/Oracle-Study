/*
 ...
 테이블 생성할 때 제약조건 이름을 설정하지 않으면, 시스템에서 자동으로 이름 부여 
 ex) SYS_C005542 -> insert할 때 제약조건에 위배가 되면 에러 메시지에서 볼 수 있음. 에러메세지의 내용을 자바로 가져올 수 있나?
...
create table ex2(
    ...
    password        varchar2(20 char) 
                         constraint ex2_password_nn_check not null 
                         constraint ex2_password_ck check (password >= 4),
    name              varchar2(100)
...
-- 테이블을 생성하면서 제약조건 이름 만들기 2: 컬럼 정의와 제약 조건 정의 분리
create table ex3(
    -- 컬럼 정의
    id                 number(4),
    name            varchar2(100),
...
-- ex_emp 테이블의 deptid 컬럼 ex)dept.detpid 컬럼을 참조 -> FK
...
create table ex_emp1 (
    ...
    deptid           number(2) -- 외래키와 맞춰주기
                        -- [constraint 이름] references 테이블 (pk인 컬럼)
                        constraint emp1_deptis_fk references ex_dept (deptid)    
);
...
-- create(생성), alter(수정), truncate(삭제), drop(삭제)
...
--> pk 컬럼이 다른 테이블에서 FK로 참조되고 있기 떄문에 삭제가 안됨
